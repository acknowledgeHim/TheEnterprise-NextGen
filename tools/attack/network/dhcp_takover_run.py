...
class DHCP_takeover:
    ...
    def nak_request(self, packet):
        ...
        dhcp_server_mac = list(self.other_dhcp_servers.keys())[0]
        dhcp_server_ip  = self.other_dhcp_servers[dhcp_server_mac]
    ...
    def check_dhcp_and_arp(self, packet):
        ...
        if DHCP in packet and packet[DHCP].options[0][1] == 3:
            ...
            if packet[Ether].src not in self.attempted_dhcpnaks:
        ...
        if ARP in packet and packet[ARP].op == 0x0002:
            if packet[Ether].src in self.macs:
