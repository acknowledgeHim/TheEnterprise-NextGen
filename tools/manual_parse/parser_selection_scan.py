...
class ParserSelection(GenericParserSelection):
    ...
    def __init__(self, db_object, full_client_engagement_path):
        try:
            ...
            print_text.print_error("tool parser selection scan except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))
        return
    ...
    def nmap_files(self):
        ...
        try:
            ...
            targets, commands = self.get_targets("nmap", self.output_folder)
            self.run_group(targets, commands, "FUNCTION:tools.manual_parse.parse_selection.parse")
        except Exception as e:
            print_text.print_error("tool parser selection scan except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))


    def ping_nmap_files(self):
        """
        All nmap scan output files.
        """
        try:
            ...
            print_text.print_error("tool parser selection scan except: " + str(e) + " Error on line {}".format(
