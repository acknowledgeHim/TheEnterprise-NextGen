...
# Modified March 2017 for Python 3 by Kevin Huber
...
from enterprise_user_conf import MSFRCP_STARTUP, MSFRPC_PORT
...
class Msfrpc:
    ...
    def __init__(self, opts=[]):
        try:
            ...
            self.client = self.host + str(self.port)
        except Exception as e:
            print_text.print_error("tool msfrcp except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        # Kill auto started msfrpc
        #print("65 msfrpc self.pid: " + str(self.pid))
        #if self.pid is not None:
        #    for proc in psutil.process_iter():
        #        print("69 msfrpc.py proc_name(): " + str(proc.name()))
        #        if "msf" in proc.name():
        #            proc.kill()
        pass
    ...
    def call(self, meth, opts=[]):
        try:
            ...
            try:
                r = requests.post(self.client + self.uri, data=params, verify=False, headers=self.headers)
                resp = r.content
                return self.decode(resp)
            except ConnectionError as e:
                # Start MSFRPC
                #proc = subprocess.Popen(shlex.split(MSFRCP_STARTUP))
                #print_text.print_msg("Automatically Starting MSFRPC, wait for it...")
                #time.sleep(120)
                #r = requests.post(self.client + self.uri, data=params, verify=False, headers=self.headers)
                #resp = r.content
                #return self.decode(resp)
...
class CredsTable(MsfTable):
    ...
    def report(self, host, port, **kwargs):
        """
        Store a set of credential in the database.
    ...
    def __init__(self, rpc, cid=None):
        ...
        try:
            ...
            if cid is None:
                ...
                if b'id' in r:
                    self.cid = r[b'id'].decode('utf-8')
                else:
                    print_text.print_error("FAILED to create console.")
            else:
                self.cid = cid
        except Exception as e:
            print_text.print_error("tool msfrpc except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))
