...
class Packer:
    ...
    def addname(self, name):
        ...
        for j in range(i):
            ...
            if offset + len(buf) < 0x3FFF:
                ...
                print_text.print_msg('DNS.Lib.Packer.addname:')
                print_text.print_error('warning: pointer too big')
            buf = buf + (chr(n) + label)
        if pointer:
    ...
    def dump(self):
        ...
        print_text.print_msg('-' * 40)
        for key in keys:
            print_text.print_msg('%20s %3d' % (key, self.index[key]))
        print_text.print_msg('-' * 40)
        space = 1
        for i in range(0, len(self.buf) + 1, 2):
            if self.buf[i:i + 2] == '**':
                ...
                continue
            space = 0
            print_text.print_msg('%4d' % i)
            for c in self.buf[i:i + 2]:
                if ' ' < c < '\177':
                    print_text.print_msg(' %c' % c)
                else:
                    print_text.print_msg('%2d' % ord(c))
        print_text.print_msg('-' * 40)
...
def testpacker():
    ...
    for i in R:
        ...
        p.addname('')
    timing.finish()
    print_text.print_msg(str(timing.milli()) + "ms total for packing")
    print_text.print_msg(str(round(timing.milli() / i, 4)) +  'ms per packing')
    # p.dump()
    u = Unpacker(p.buf)
    u.getaddr()
    u.getbytes(20)
    ...
    for i in R:
        ...
        res = (u.getaddr(),
               ...
               u.getname())
    timing.finish()
    print_text.print_msg(str(timing.milli()) + "ms total for unpacking")
    print_text.print_msg(str(round(timing.milli() / i, 4)) + 'ms per unpacking')
...
def dumpM(u):
    print_text.print_msg('HEADER:')
    (id, qr, opcode, aa, tc, rd, ra, z, rcode,
     qdcount, ancount, nscount, arcount) = u.getHeader()
    print_text.print_msg('id=%d,' % id)
    print_text.print_msg('qr=%d, opcode=%d, aa=%d, tc=%d, rd=%d, ra=%d, z=%d, rcode=%d,' \
        % (qr, opcode, aa, tc, rd, ra, z, rcode))
    if tc:
        print_text.print_msg('*** response truncated! ***')
    if rcode:
        print_text.print_msg('*** nonzero error code! (%d) ***' % rcode)
    print_text.print_msg('  qdcount=%d, ancount=%d, nscount=%d, arcount=%d' \
        % (qdcount, ancount, nscount, arcount))
    for i in range(qdcount):
        print_text.print_msg('QUESTION %d:' % i)
        dumpQ(u)
    for i in range(ancount):
        print_text.print_msg('ANSWER %d:' % i)
        dumpRR(u)
    for i in range(nscount):
        print_text.print_msg('AUTHORITY RECORD %d:' % i)
        dumpRR(u)
    for i in range(arcount):
        print_text.print_msg('ADDITIONAL RECORD %d:' % i)
        dumpRR(u)
...
class DnsResult:
    ...
    def show(self):
        import time
        print_text.print_msg('; <<>> PDG.py 1.0 <<>> %s %s' % (self.args['name'],
                                                self.args['qtype']))
        opt = ""
        if self.args['rd']:
            opt = opt + 'recurs '
        h = self.header
        print_text.print_msg(';; options: ' + opt)
        print_text.print_msg(';; got answer:')
        print_text.print_msg(';; ->>HEADER<<- opcode %s, status %s, id %d' % (
            h['opcode'], h['status'], h['id']))
        flags = list(filter(lambda x, h=h: h[x], ('qr', 'aa', 'rd', 'ra', 'tc')))
        print_text.print_msg(';; flags: %s; Ques: %d, Ans: %d, Auth: %d, Addit: %d' % (
            string.join(flags), h['qdcount'], h['ancount'], h['nscount'],
            h['arcount']))
        print_text.print_msg(';; QUESTIONS:')
        for q in self.questions:
            print_text.print_msg(';;      %s, type = %s, class = %s' % (q['qname'], q['qtypestr'],
                                                         q['qclassstr']))

        print_text.print_msg(';; ANSWERS:')
        for a in self.answers:
            print_text.print_msg('%-20s    %-6s  %-6s  %s' % (a['name'], repr(a['ttl']), a['typename'],
                                               a['data']))

            print_text.print_msg(';; AUTHORITY RECORDS:')
        for a in self.authority:
            print_text.print_msg('%-20s    %-6s  %-6s  %s' % (a['name'], repr(a['ttl']), a['typename'],
                                               a['data']))

        print_text.print_msg(';; ADDITIONAL RECORDS:')
        for a in self.additional:
            print_text.print_msg('%-20s    %-6s  %-6s  %s' % (a['name'], repr(a['ttl']), a['typename'],
                                               a['data']))

        if 'elapsed' in self.args:
            print_text.print_msg(';; Total query time: %d msec' % self.args['elapsed'])
        print_text.print_msg(';; To SERVER: %s' % (self.args['server']))
        print_text.print_msg(';; WHEN: %s' % time.ctime(time.time()))
...
def dumpQ(u):
    ...
    print_text.print_msg('qname=%s, qtype=%d(%s), qclass=%d(%s)' \
        % (qname,
...
def dumpRR(u):
    ...
    print_text.print_msg('name=%s, type=%d(%s), class=%d(%s), ttl=%d' \
        % (name,
           type, typename,
    ...
    if hasattr(u, mname):
        print_text.print_msg('  formatted rdata:', getattr(u, mname)())
    else:
        print_text.print_msg('  binary rdata:', u.getbytes(rdlength))
