...
This code is covered by the standard Python License.
    ...
    Modified March 14, 2017 by 'huber' for python3
"""
...
def ParseResolvConf(resolv_path):
    ...
    try:
        ...
        print_text.print_error("theharvester error in path" + resolv_path)
    for line in lines:
        ...
        for i in list(defaults.keys()):
            if i not in args:
                if i in self.defaults:
                    ...
                    args[i] = defaults[i]
        if isinstance(args['server'], bytes):
...
#                Lib.dumpM(u)
    ...
    def req(self, *name, **args):
        ...
        if isinstance(self.args['qtype'], bytes):
            try:
        ...
        if 'name' not in self.args:
            print_text.print_msg(self.args)
            raise DNSError('nothing to lookup')
        qname = self.args['name']
        if qtype == Type.AXFR:
            print_text.print_msg('Query type AXFR, protocol forced to TCP')
            protocol = 'tcp'
        # print 'QTYPE %d(%s)' % (qtype, Type.typestr(qtype))
        m = Lib.Mpacker()
        #keywords and default args would be good. TODO.
        m.addHeader(0,
