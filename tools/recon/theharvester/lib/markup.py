...
class page:
    ...
    def __init__(self, mode='strict_html', case='lower',
        ...
        if mode == 'strict_html' or mode == 'html':
            ...
            self.onetags += list(item.lower() for item in self.onetags)
            self.twotags = valid_twotags
            self.twotags += list(item.lower() for item in self.twotags)
            self.deptags = deprecated_onetags + deprecated_twotags
            self.deptags += list(item.lower() for item in self.deptags)
