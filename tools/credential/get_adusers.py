        ...
        for item in resp:
            ...
            try:
                for attribute in item['attributes']:
                    ...
                    elif attribute['type'] == 'Account-Expires':
                        account_expires = str(attribute['vals'][0])
                        if account_expires == "0" or account_expires == "0x7FFFFFFFFFFFFFFF":
                            account_expires = "Never"

                answers.append([sAMAccountName, mail, pwdLastSet, lastLogon, account_expires])
            except Exception as e:
                logging.error('Skipping item, cannot process due to error %s' % str(e))
                pass

        if len(answers)>0:
            self.printTable(answers, header=[ "Name", "Email", "PasswordLastSet", "LastLogon", "AccountExpires"])
            print('\n\n')
