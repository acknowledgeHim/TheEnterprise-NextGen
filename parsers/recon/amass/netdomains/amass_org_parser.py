...
def amass(db_path, db_object, key, hashvals):
    ...
    """
    try:
        with Parser("amass_netdomains-org", db_object, key, hashvals, False) as p:
            p.parse("parsers.recon.amass.netdomains.amass_org_parser", "AmassParser")

    except Exception as e:
        print("amass_org_parser 23 except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))
...
class AmassParser():
    ...
    def __init__(self, db_object, location_id, scope_id, file_path_name, ext, just_file_name, target, log_id, output_path, tester_device_list, modified_by, modified_date):
        ...
        self.tool = "amass_netdomains-org"
        self.target = target
        self.log_id = log_id
        self.ext = ext
