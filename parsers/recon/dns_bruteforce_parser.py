...
def dns(db_path, db_object, key, hashvals):
    ...
    """
    try:
        with Parser("dns bruteforce", db_object, key, hashvals, False) as p:
            p.parse("parsers.recon.dns_bruteforce_parser", "DNSBruteforceParser")

    except Exception as e:
        print("dns_bruteforce parser 23 except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))


class DNSBruteforceParser():
    """ Parse DNSBruteforce files. """
    ...
    def parse(self):
        ...
        try:
            ...
            print("dns bruteforce parser 75 except: " + str(e) + " Error on line {}".format(sys.exc_info()[-1].tb_lineno))
        return {}
